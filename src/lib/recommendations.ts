...
export function getRecommendedArticles(limit = 20, offset = 0): Article[] {
  ...
  if (weights.length === 0) return getLatestArticles(limit, offset);
  ...
  const scored = articles.map(article => {
    const articleTags = db.prepare(`SELECT tag_id FROM article_tags WHERE article_id = ?`).all(article.id) as { tag_id: number }[];
    let tagScore = 0;
    for (const { tag_id } of articleTags) {
    ...
    const ratingPenalty = article.rating_count > 0 ? 0.3 : 0;
    article.score = tagScore * 2 + recencyBonus - ratingPenalty;
    return article;
  });

  scored.sort((a, b) => (b.score || 0) - (a.score || 0));
  return scored.slice(offset, offset + limit).map(article => ({ ...article, tags: getArticleTags(article.id) }));
}
...
export function getTopPick(period: 'week' | 'month'): Article | null {
  ...
  const row = db.prepare(`
    ...
    ORDER BY score DESC
    LIMIT 1
  `).get() as (Article & { avg_rating: number; rating_count: number }) | undefined;
  if (!row) return null;
...
export function rateArticle(articleId: number, rating: number): void {
  ...
  if (existing) {
    db.prepare("UPDATE ratings SET rating = ?, created_at = datetime('now') WHERE article_id = ?").run(rating, articleId);
  } else {
